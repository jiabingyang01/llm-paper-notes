...
笔记应包含：
1. 标题 + 元信息（作者、机构、时间、链接、期刊/会议）——需联网检索论文是否已被期刊或会议录用，如已中稿则注明发表venue
2. 一句话总结
...
**禁止在代码块（\`\`\`）中书写算法伪代码或流程描述**。伪代码中的数学符号（下标、上标、希腊字母、矩阵等）必须使用 LaTeX 渲染，应使用 blockquote + 有序列表 + 行内 LaTeX 的形式呈现，确保所有变量和公式在页面上正确显示。
...
- 不使用 emoji（除非用户要求）
- 元信息 blockquote：每条 `>` 行之间必须插入一个空的 `>` 行，否则 VitePress 会将所有内容渲染为同一段落。正确格式：`> **论文**：...\n>\n> **作者**：...`

## 自我迭代规则

当用户指出笔记中的格式或内容问题并要求纠正时，修复完成后**必须**同步在本文件（CLAUDE.md）的相应章节中补充或更新对应的规范条目，确保同类问题不再重复出现。
