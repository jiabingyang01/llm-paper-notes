...
对每层 ℓ ∈ L:
  1. 提取隐状态嵌入 E⁺, E⁻
  2. 计算注意力矩阵 A⁺, A⁻ → 头平均 → Ā⁺, Ā⁻
  3. 计算位置注意力分布 π⁺, π⁻（列均值）
  4. 注意力加权特征 Z⁺ = π⁺·E⁺, Z⁻ = π⁻·E⁻
  5. 差值矩阵 Z = Z⁺ − Z⁻
  6. SVD(Z) → 取前 k 个右奇异向量 V_k
  7. 幻觉投影算子 P = V_k · V_k^T
  8. 加权零空间算子 N = I − HIS^c · P
  9. 编辑 MLP 权重：W_up^ed = N · W_up, W_down^ed = W_down · N
