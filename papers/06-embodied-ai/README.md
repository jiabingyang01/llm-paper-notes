...
| 论文 | 关键词 | 年份 |
| --- | --- | --- |
| [3D-CAVLA](vla/perception/3D_CAVLA_2025.md) | 3D 深度感知、CoT 指令分解、ROI 检测、零样本泛化、LIBERO | 2025 |
| [3D-MIX](vla/perception/3D_Mix_2026.md) | VGGT 3D 融合、9 种策略对比、语义条件化门控、即插即用、GR00T/π-style、SIMPLER +7.0% | 2026 |
| [AimBot](vla/perception/AimBot_2025.md) | 瞄准线 + 准星视觉空间线索、EE 位姿/朝向/夹爪编码到像素空间、<1 ms 模型无关、LIBERO-Long +5.8、真实世界 27→43/50 | 2025 |
| [AnchorVLA4D](vla/perception/AnchorVLA4D_2026.md) | 首帧锚帧 + 冻结 Any4D 空间编码器、遮挡遗忘缓解、早期重试、SimplerEnv 64.6%（+13.6%）、真实世界 80% | 2026 |
| [BridgeVLA](vla/perception/BridgeVLA_2025.md) | 输入-输出 2D 对齐、正交投影、2D 热力图预训练、RLBench 88.2%、3 条轨迹 95.4% | 2025 |
| [DeepVision-VLA](vla/perception/DeepVisionVLA_2026.md) | 视觉敏感性衰减诊断、VL-MoT DINOv3 视觉专家深层耦合、AGVP 动作引导 Token 剪枝、RLBench 83%、真实世界 91.7% | 2026 |
| [SF](vla/perception/SF_2025.md) | 隐式空间表征对齐（VGGT）、中间层视觉 embedding 监督、推理零开销、3.8× 训练加速、LIBERO 98.5% | 2025 |
| [TGM-VLA](vla/perception/TGM_VLA_2026.md) | 关键帧采样优化、颜色反转投影、跨任务/任务内 Mixup、RLBench 90.5%、COLOSSEUM 68.8% | 2026 |
| [UAOR](vla/perception/UAOR_2026.md) | 观测遗忘诊断、Action Entropy 不确定性检测、观测特征 FFN 重注入、Training-Free、Plug-and-Play | 2026 |
| [VP-VLA](vla/perception/VP_VLA_2026.md) | 双系统架构、视觉提示接口（十字准星+边框）、事件驱动任务分解、视觉接地辅助损失、RoboCasa +5%、SimplerEnv +8.3% | 2026 |
...
| 论文 | 关键词 | 年份 |
| --- | --- | --- |
| [CoWVLA](vla/reasoning/CoWVLA_2026.md) | 结构-运动解耦 Video VAE、潜在运动链、Chain-of-World、终端帧预测、LIBERO 95.6%、SimplerEnv 76.0%、CALVIN 4.21 | 2026 |
...
| [MoH](vla/reasoning/MoH_2025.md) | 多 Horizon 动作块并行融合、轻量门控（2k 参数）、跨 Horizon 共识动态推理、Plug-and-Play、LIBERO 99% | 2025 |
| [OptimusVLA](vla/reasoning/OptimusVLA_2026.md) | 双记忆增强（GPM 任务级先验检索 + LCM Mamba 时序一致性）、自适应 NFE、LIBERO 98.6%、2.9× 推理加速 | 2026 |
| [ProgressVLA](vla/reasoning/ProgressVLA_2026.md) | 进度估计、Classifier Guidance、潜在动作空间、世界模型、扩散策略、长时域操作 | 2026.03 |
| [SPR](vla/reasoning/SPR_2026.md) | 进度感知空间子目标规划、See-Plan-Rewind 闭环、自主错误恢复、LIBERO 91.8%、LIBERO-Plus OOD ↓18.8% | 2026 |
| [TCoT](vla/reasoning/TCoT_2026.md) | 全局/局部轨迹思维链、GLSR 失败检测与策略切换恢复、跨任务知识共享、LIBERO 83.3%（Multi）、真实世界 +28% | 2026 |
...
| [FocusVLA](vla/efficient/FocusVLA_2026.md) | Modality Cascaded Attention + Focus Attention（Patch 剪枝 + Channel 门控）、视觉利用效率瓶颈诊断、0.5B 超越 7B、LIBERO 98.7%、训练 1.5× 加速 | 2026 |
| [GridS](vla/efficient/GridS_2026.md) | 可微双线性采样、连续坐标预测、几何感知剪枝、端到端联合训练、ICML 2026 | 2026 |
...
| [ViVa](vla/rl/ViVa_2026.md) | 视频生成价值模型、Wan2.2 DiT、Latent Injection、未来本体感知预测、RECAP value 替换 | 2026 |
...
| [WMPO](vla/rl/WMPO_2025.md) | 像素级视频世界模型、OpenSora + Policy Behavior Alignment、On-policy GRPO、OpenVLA-OFT 微调 | 2025 |
...
纯世界模型 / 模拟器：预测未来状态（$p(o'\mid o,a)$），作数据生成、RL 训练场或策略评估用，本身不联合产出策略。
...
| [MIND-V](world-models/MINDV_2026.md) | 认知分层 VWM（SRH + BSB + MVG）、CogVideoX-5B 底座、V-JEPA2 物理裁判 PFC 奖励、GRPO 后训练、Staged Visual Future Rollouts、PFC +9.0%、Task Success +76.7% | 2026 |

---

## 🌐 World Action Models

世界动作模型（WAM）：联合建模未来状态与动作（$p(o',a\mid o,l)$），世界预测与动作生成 co-equal，直接产出策略（VAM 视为 WAM 子集）。区别于"世界模型辅助的 VLA 策略"（那类归 VLA 推理与规划）——判据是世界建模是否为一级输出。

| 论文 | 关键词 | 年份 |
| --- | --- | --- |
| [Fast-WAM](world-action-models/FastWAM_2026.md) | 视频协同训练 vs. 测试时未来想象受控拆解、MoT 架构（Wan2.2-5B + Action DiT）、训练-推理解耦、190 ms 延迟、RoboTwin 91.8%、LIBERO 97.6% | 2026.03 |
| [LDA-1B](world-action-models/LDA_1B_2026.md) | 统一世界模型基础模型（策略+正逆动力学+视觉预测）、通用数据摄入（质量分层监督）、结构化 DINO 潜在预测、MM-DiT、EI-30k、RoboCasa-GR1 55.4% 超 3B GR00T、真机 +21%/48%/23%、RSS 2026 | 2026 |
| [SpatialVAM](world-action-models/SpatialVAM_2026.md) | 3D Video Action Model、多视图正交投影 RGB + 热力图、Wan2.2-5B + View-Attention LoRA、双扩散损失、Meta-World 89.1%、RoboCasa 42%、真实 Franka 57.1%、单步去噪 85.7% | 2026 |
| [WAM Survey](world-action-models/WAM_Survey_2026.md) | 首篇 World Action Models 系统综述、定义 WAM 与相邻概念划界、Cascaded/Joint 架构分类、四类数据生态、三维评估协议（视觉保真/物理常识/动作可行性）、七大开放挑战 | 2026 |
| [WorldVLA](world-action-models/WorldVLA_2025.md) | 自回归统一动作+世界模型、Chameleon 骨架、Action Attention Mask 阻断误差累积、VQ-GAN 离散 token、LIBERO 81.8% | 2025 |
...
| 论文 | 关键词 | 年份 |
| --- | --- | --- |
| [MimicDreamer](data-synthesis/MimicDreamer_2025.md) | Human-to-Robot、视频扩散、EgoStabilizer 稳视点、IK 动作对齐（软屏蔽 roll）、H2R Aligner（CogVideoX-5b 换手）、π₀ 后训练、EgoDex、六任务平均 +14.7% SR | 2025 |
| [Wh0](data-synthesis/Wh0_2026.md) | 生成式世界模型作可控人手数据引擎、5 万条 WM-H、Wan-I2V + Qwen-Image-Edit、场景/具身对齐、HaWoR + MANO、VITRA 共训、Unitree G1 灵巧手 8.3%→38.9%、解锁预训练先验 | 2026 |
