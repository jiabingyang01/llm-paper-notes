...
|:---|:---|:---|:---|:---|
| 世界模型空间 | **像素空间**（视频预测） | **像素空间**（OpenSora 视频扩散） | 隐空间 | 像素空间 |
| 奖励来源 | Verified reward（MAE+LPIPS） | VideoMAE 二分类 reward model | 想象轨迹 reward | VLM 判别 |
| 策略优化 | GRPO | On-policy GRPO（去 KL，follow DAPO） | PPO | PPO |
| 策略参数化 | SDE-Policy（Flow+Sigma） | OpenVLA-OFT（离散动作 token） | Diffusion | Diffusion |
| 训练步数 | **400** | ~数千 | ~数千 | ~数千 |

VLA-RFT 的最大特色是在**像素空间**做世界模型预测，配合 verified reward 实现极高的样本效率。WMPO 也在像素空间做 RL，但奖励是学习的二分类成功判别器而非 verified reward。
...
WMPO 与 VLA-RFT 都在**像素空间**做世界模型 + GRPO，但奖励设计与策略参数化差异明显：

- **奖励**：VLA-RFT 用 verified reward（MAE+LPIPS 的同空间对比）可验证、无需训练；WMPO 用 VideoMAE 二分类成功判别器（F1>0.95 但仍是学习到的）——前者更可解释，后者更通用
- **策略形式**：VLA-RFT 的 SDE-Policy 基于 flow matching，WMPO 基于 OpenVLA-OFT 的离散 action token，二者对应不同 GRPO 似然比计算路径
- **训练步数**：VLA-RFT 400 步极端高效可能得益于像素级 dense reward 的信号丰富度；WMPO 用稀疏二分类奖励 + 长想象轨迹
...
- **WMPO**（Zhu et al., 2025）：像素级视频世界模型（OpenSora）+ on-policy GRPO 微调 OpenVLA-OFT，本站有[笔记](./WMPO_2025.md)
